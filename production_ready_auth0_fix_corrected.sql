-- Production-Ready Auth0 to Supabase Integration (CORRECTED)
...
-- Allow all operations on mapping table (it's just a mapping)
...
CREATE OR REPLACE FUNCTION get_or_create_supabase_user_for_auth0(
    auth0_user_id TEXT,
    auth0_email TEXT DEFAULT NULL,
    auth0_name TEXT DEFAULT NULL,
    auth0_nickname TEXT DEFAULT NULL,
    auth0_picture TEXT DEFAULT NULL
)
RETURNS UUID AS $$
DECLARE
    existing_mapping auth0_user_mappings%ROWTYPE;
    new_supabase_user_id UUID;
BEGIN
    -- Check if mapping already exists
    SELECT * INTO existing_mapping 
    FROM auth0_user_mappings 
    WHERE auth0_user_id = get_or_create_supabase_user_for_auth0.auth0_user_id;
    
    -- If mapping exists, return the Supabase user ID
    IF existing_mapping.supabase_user_id IS NOT NULL THEN
    ...
    INSERT INTO auth.users (
        ...
        gen_random_uuid(),
        COALESCE(auth0_email, auth0_user_id || '@auth0.local'),
        NOW(),
        NOW(),
        NOW(),
        '{"provider": "auth0", "providers": ["auth0"]}'::jsonb,
        jsonb_build_object(
            'name', COALESCE(auth0_name, auth0_nickname, 'Auth0 User'),
            'nickname', COALESCE(auth0_nickname, auth0_name, 'auth0user'),
            'picture', auth0_picture
        ),
        false,
        '' -- No password for Auth0 users
    ) RETURNING id INTO new_supabase_user_id;
    ...
    RETURN new_supabase_user_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Step 3: Create function to get Supabase user ID from Auth0 user ID
CREATE OR REPLACE FUNCTION get_supabase_user_id_from_auth0(auth0_user_id TEXT)
RETURNS UUID AS $$
DECLARE
    mapped_user_id UUID;
BEGIN
    ...
    FROM auth0_user_mappings
    WHERE auth0_user_id = get_supabase_user_id_from_auth0.auth0_user_id;
...
-- Tracking metrics table policies (CORRECTED - uses pet_id, not user_id)
...
-- Tracking entries table policies (CORRECTED - uses metric_id, not user_id)
...
CREATE POLICY "Users can view own pet tracking entries" ON tracking_entries
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM tracking_metrics tm
            JOIN pets p ON p.id = tm.pet_id
