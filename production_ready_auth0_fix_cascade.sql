-- Production-Ready Auth0 to Supabase Integration (CASCADE - DROPS POLICIES FIRST)
...
-- Step 2: Drop ALL policies first (they depend on functions)
-- Pets table policies
...
-- Tracking entries table policies
...
-- Step 3: Now drop existing functions (no dependencies)
...
-- Step 4: Create functions with new parameter names
...
BEGIN
    ...
    FROM auth.users 
    WHERE email = COALESCE(p_auth0_email, p_auth0_user_id || '@auth0.local');
...
-- Create function to get Supabase user ID from Auth0 user ID
...
-- Step 5: Now create all policies with the new functions
-- Pets table policies
