-- DEFINITIVE AUTH SCHEMA FIX - CORRECTED FOR YOUR ACTUAL TABLE STRUCTURE
-- This fixes ALL the root causes using your actual column names
...
-- Pets - users can manage their own pets (using 'id' not 'user_id')
CREATE POLICY "pets_select" ON pets FOR SELECT USING (id = get_current_user_id());
CREATE POLICY "pets_insert" ON pets FOR INSERT WITH CHECK (id = get_current_user_id());
CREATE POLICY "pets_update" ON pets FOR UPDATE USING (id = get_current_user_id());
CREATE POLICY "pets_delete" ON pets FOR DELETE USING (id = get_current_user_id());

-- Shopping items - users can manage their own items (using 'id' not 'user_id')
CREATE POLICY "shopping_select" ON shopping_items FOR SELECT USING (id = get_current_user_id());
CREATE POLICY "shopping_insert" ON shopping_items FOR INSERT WITH CHECK (id = get_current_user_id());
CREATE POLICY "shopping_update" ON shopping_items FOR UPDATE USING (id = get_current_user_id());
CREATE POLICY "shopping_delete" ON shopping_items FOR DELETE USING (id = get_current_user_id());

-- Posts - all can view, users can manage their own (using 'user_id' for posts table)
...
-- Profiles - users can manage their own profile (using 'id' not 'user_id')
CREATE POLICY "profiles_select" ON profiles FOR SELECT USING (id = get_current_user_id());
CREATE POLICY "profiles_insert" ON profiles FOR INSERT WITH CHECK (id = get_current_user_id());
CREATE POLICY "profiles_update" ON profiles FOR UPDATE USING (id = get_current_user_id());
CREATE POLICY "profiles_delete" ON profiles FOR DELETE USING (id = get_current_user_id());

-- Comments - all can view, users can manage their own (using 'id' not 'user_id')
CREATE POLICY "comments_select_all" ON comments FOR SELECT USING (true);
CREATE POLICY "comments_insert" ON comments FOR INSERT WITH CHECK (id = get_current_user_id());
CREATE POLICY "comments_update" ON comments FOR UPDATE USING (id = get_current_user_id());
CREATE POLICY "comments_delete" ON comments FOR DELETE USING (id = get_current_user_id());

-- Tracking - users can manage their own data (using 'id' not 'user_id')
CREATE POLICY "tracking_metrics_select" ON tracking_metrics FOR SELECT USING (id = get_current_user_id());
CREATE POLICY "tracking_metrics_insert" ON tracking_metrics FOR INSERT WITH CHECK (id = get_current_user_id());
CREATE POLICY "tracking_metrics_update" ON tracking_metrics FOR UPDATE USING (id = get_current_user_id());
CREATE POLICY "tracking_metrics_delete" ON tracking_metrics FOR DELETE USING (id = get_current_user_id());
...
BEGIN
    ...
    FROM profiles
    WHERE id = v_supabase_user_id;
    
    RAISE NOTICE 'Deleting all data for user: % (Auth0: %)', v_supabase_user_id, p_auth0_user_id;
    
    -- Delete all user data (using correct column names)
    DELETE FROM tracking_entries WHERE metric_id IN (
        SELECT id FROM tracking_metrics WHERE id = v_supabase_user_id
    );
    DELETE FROM tracking_metrics WHERE id = v_supabase_user_id;
    DELETE FROM comments WHERE id = v_supabase_user_id;
    DELETE FROM pets WHERE id = v_supabase_user_id;
    DELETE FROM shopping_items WHERE id = v_supabase_user_id;
    DELETE FROM posts WHERE user_id = v_supabase_user_id;
    DELETE FROM profiles WHERE id = v_supabase_user_id;
...
BEGIN
    ...
    -- Delete all user data (using correct column names)
    DELETE FROM tracking_entries WHERE metric_id IN (
        SELECT id FROM tracking_metrics WHERE id = v_user_id
    );
    DELETE FROM tracking_metrics WHERE id = v_user_id;
    DELETE FROM comments WHERE id = v_user_id;
    DELETE FROM pets WHERE id = v_user_id;
    DELETE FROM shopping_items WHERE id = v_user_id;
...
SELECT '2. Fixed RLS policies to use correct column names' as change;
